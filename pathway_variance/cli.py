"""
Command-Line Interface for PathwayVariance: Enhanced Recovery After Surgery (ERAS) Adherence Agent.
"""
import argparse
import csv
import json
...
from .agents import PathwayVarianceCoordinator

coordinator = PathwayVarianceCoordinator()


def main(argv=None):
    parser = argparse.ArgumentParser(prog="clinical-pathway-variance-agent", description="PathwayVariance: Enhanced Recovery After Surgery (ERAS) Adherence Agent")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  PATHWAYVARIANCE: ENHANCED RECOVERY AFTER SURGERY (ERAS) ADHERENCE AGENT")
        print(f"  Domain: Clinical Operations | Standard: ERAS Society Perioperative Care Guidelines")
    ...
    if args.command == "serve":
        try:
            ...
            app = create_app()
            if app:
                print(f"Starting PathwayVariance: Enhanced Recovery After Surgery (ERAS) Adherence Agent on http://{args.host}:{args.port}")
